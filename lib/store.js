...
function load() {
  try {
    return mergeSeed(JSON.parse(fs.readFileSync(DATA_PATH, "utf8")));
  } catch {
...
export function listBoard(board = "all", category = null) {
  ...
  let rows = state.listings.map((l) => ({
    ...l,
    window_amount: windowAmount(state, l, board),
  }));
  if (category) rows = rows.filter((r) => r.category_slug === category);
  if (board !== "all") rows = rows.filter((r) => r.window_amount > 0);
  rows.sort((a, b) => b.window_amount - a.window_amount || new Date(a.created_at) - new Date(b.created_at));
