...
test("officialSocials exposes official TikTok, Instagram and email", () => {
  ...
  assert.equal(OFFICIAL_INSTAGRAM_HANDLE, "@podiosync_oficial");
  assert.equal(OFFICIAL_INSTAGRAM_URL, "https://www.instagram.com/podiosync_oficial/");
  assert.equal(OFFICIAL_EMAIL, "podio@podiosync.es");
  assert.equal(OFFICIAL_EMAIL_HREF, "mailto:podio@podiosync.es");
  assert.match(html, /href="https:\/\/www\.tiktok\.com\/@podiosync"/);
  assert.match(html, /href="https:\/\/www\.instagram\.com\/podiosync_oficial\/"/);
  assert.match(html, /href="mailto:podio@podiosync.es"/);
  assert.match(html, /aria-label="TikTok oficial de PodioSync, @podiosync"/);
  assert.match(html, /aria-label="Instagram oficial de PodioSync, @podiosync_oficial"/);
  assert.match(html, /aria-label="Correo de PodioSync, podio@podiosync.es"/);
  assert.match(html, />@podiosync</);
  assert.match(html, />@podiosync_oficial</);
  assert.match(html, />podio@podiosync\.es</);
  assert.match(
    html,
    /href="https:\/\/www\.instagram\.com\/podiosync_oficial\/"[^>]*target="_blank"[^>]*rel="noopener noreferrer"/,
  );
  assert.doesNotMatch(html, /href="mailto:podio@podiosync.es"[^>]*target=/);
  assert.doesNotMatch(html, /x\.com|twitter\.com/i);
});
...
test("launch copy is production Spanish without demo framing", () => {
  ...
  const about = aboutPage({ revenue: 1, listings: 1, visitors: 1 });
  assert.match(about, /Síguenos/);
  assert.match(about, /tiktok\.com\/@podiosync/i);
  assert.match(about, /instagram\.com\/podiosync_oficial/i);
  assert.match(about, /mailto:podio@podiosync\.es/);
